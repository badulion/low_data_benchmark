...
class PointTransformerV3(PointModule):
    ...
    def forward(self, x, p):
        ...
        data_dict.batch = torch.arange(x.shape[0], device=x.device).repeat_interleave(x.shape[1])
        data_dict.coord = torch.flatten(torch.cat((p,torch.zeros((p.shape[0],p.shape[1],1), device=p.device)), dim=-1), start_dim=0, end_dim=1)
