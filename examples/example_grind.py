...
train_loader = DataLoader(burgers_train_iterator, batch_size=16, shuffle=True)

prediction_net = NeuralPDE(input_dim=1, hidden_channels=64, hidden_layers=3,
                solver={'method': 'euler', 'options': {'step_size': 0.1}},
                use_adjoint=False)

model = GrIND(prediction_net, num_ks=9, grid_resolution=64, spatial_dim=2)

model = PointIterativeWrapper(model)
...
test_loader = DataLoader(burgers_test_iterator, batch_size=16, shuffle=False)
