...
if [[ -f "$STATUS_FILE" ]] && grep -q "TRAINING_COMPLETED" "$STATUS_FILE"; then
    ...
    cp -r "/home/atuin/b190cb/b190cb18/data/{equation}/cloud/{resolution}" "$TMPDIR/cloud/{equation}/grid"
    ...
    # start job
    srun python main.py model={model} equation={equation} resolution={resolution} version={version} Batch_size={batch} Epochs={epochs} Base_path_data=$TMPDIR/data output_dir={output_path}
